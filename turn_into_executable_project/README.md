...
First of all, an executable file is an **environement independant** file that, in a system that doesn't have any project dependencies installed, **not even Python**, would run perfectly. In my case, anyone taking my file ***dist/Temp_viz*** (It's actually zipped because of its large size, ~270MB) and running it on a debian machine will "hopefully" get my app popping-up for them.
